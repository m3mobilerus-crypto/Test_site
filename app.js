...
function loadInlineStockData(sku, bodyId) {
  ...
  if (window.treolanLoaded && window.TREOLAN_DATA) {
    const row = window.TREOLAN_DATA.find(r => norm(r.articul) === skuU);
    if (row) {
  ...
  if (window.ocsLoaded && window.OCS_DATA) {
    const row = window.OCS_DATA.find(r => norm(r.articul) === skuU);
    if (row) {
      results.push({dist:'OCS', color:'#10b981', qty: parseInt(row.quantity||row.stock||0)||0, transit:0});
    } else {
