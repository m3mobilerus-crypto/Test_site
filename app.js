...
function loadInlineStockData(sku, bodyId) {
  ...
  // Check OCS — uses item.articul
  if (window.ocsLoaded && window.OCS_DATA) {
    // Try exact norm match first, then partial match
    let row = window.OCS_DATA.find(r => norm(r.articul) === skuU);
    if (!row) {
      // Try: OCS may store articul with different separators or extra spaces
      row = window.OCS_DATA.find(r => {
        const a = norm(r.articul || r['Артикул'] || r['articul'] || r['sku'] || '');
        return a === skuU;
      });
    }
    if (!row) {
      ...
      row = window.OCS_DATA.find(r => {
        return Object.values(r).some(v => typeof v === 'string' && norm(v) === skuU);
      });
    }
    if (row) {
      const qty = parseInt(row.quantity || row.stock || row['quantity'] || row['stock'] || row['qty'] || 0) || 0;
      results.push({dist:'OCS', color:'#10b981', qty, transit:0});
      console.log('OCS found:', row, 'qty:', qty);
    } else {
      results.push({dist:'OCS', color:'#10b981', qty:0, transit:0, notFound:true});
      // Debug: show sample OCS records
      console.log('OCS miss for skuU:', skuU, 'sample:', window.OCS_DATA.slice(0,3).map(r=>({articul:r.articul, keys:Object.keys(r)})));
    }
  }

  // Check M3 Stock — uses item['артикул'] || item['sku'] || item['articul']
  if (window.m3StockLoaded && window.M3_STOCK_DATA) {
    const row = window.M3_STOCK_DATA.find(r => {
      const a = norm(r['артикул'] || r['sku'] || r['articul'] || Object.values(r)[0] || '');
      return a === skuU;
    });
    if (row) {
      const qty = Object.values(row).find((v,i) => i > 0 && !isNaN(parseInt(v)));
      results.push({dist:'M3 Stock', color:'#f59e0b', qty: parseInt(qty||0)||0, transit:0});
    } else {
